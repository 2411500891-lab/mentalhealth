...
COLOR_MAP_URGENCY = {
    "🚨 Butuh Pertolongan Segera": C_SOFTRED,
...
# Urgensi: Butuh Pertolongan Segera vs Curhat Ringan (sesuai instruksi dosen)
...
MODERATE_URGENT_PHRASES = {
    ...
    "ga kuat lagi","tidak kuat lagi","trauma","burnout parah","stress berat","stres berat",
...
MILD_PHRASES = {
    ...
    "khawatir","galau","overthinking","overthink","burnout",
}

# Klaster akar masalah sesuai instruksi dosen: tekanan akademik, masalah keluarga, finansial
CLUSTER_KEYWORDS = {
    "Tekanan Akademik": {
        "tugas","skripsi","ujian","dosen","kuliah","tenggat waktu","ipk","krs",
        "sidang","praktikum","semester","kampus","magang","nilai","revisi","sks",
        "burnout","kuliah","prodi","matkul","presentasi","laporan",
    },
    "Masalah Keluarga": {
        "orang tua","ayah","ibu","keluarga","broken home","kdrt",
        "cerai","dibanding bandingkan","tekanan keluarga","ekspektasi",
        "berantem","dipaksa","ortu","rumah","orang tua",
    },
    "Finansial": {
        "uang","biaya kuliah","biaya","ekonomi","kerja","gaji","utang","beasiswa",
        "finansial","susah cari kerja","tidak punya uang","nunggak","kost","makan",
        "ongkos","bayar","cicilan",
    },
    "Hubungan Sosial": {
        "pacar","putus","mantan","teman","lingkar pertemanan","sahabat",
    ...
    "Kesehatan Mental Umum": {
        "depresi","cemas","anxiety","stress","stres","burnout","trauma",
...
# ============================================================
# KLASIFIKASI URGENSI
# Label disesuaikan: Butuh Pertolongan Segera vs Curhat Ringan
# ============================================================
def score_urgency(raw_text_lower):
    ...
    if severe >= 1 or score >= 4:
        label = "🚨 Butuh Pertolongan Segera"
...
# ============================================================
# AUTO-DETEKSI KOLOM (disesuaikan dengan kolom CSV burnoutkuliah)
# ============================================================
...
def run_kmeans(texts, k=3):
    ...
    n_svd_components = max(1, min(2, n_docs - 1, X_tfidf.shape[1] - 1))
    svd = TruncatedSVD(n_components=n_svd_components, random_state=42)
    X_2d = svd.fit_transform(X_tfidf)

    wcss = []
    k_range = range(2, max(2, min(7, n_docs)))
    for ki in k_range:
        if ki > n_docs:
            break
    ...
    km_final = KMeans(n_clusters=k, random_state=42, n_init=min(10, n_docs))
    labels = km_final.fit_predict(X_2d)

    sil = silhouette_score(X_2d, labels) if len(set(labels)) > 1 and n_docs > len(set(labels)) else 0
...
def generate_urgency_insight(df):
    total = len(df)
    n_darurat = (df["urgensi"] == "🚨 Butuh Pertolongan Segera").sum()
    ...
    bullets = [
        f"<b>{pct_darurat:.1f}%</b> dari seluruh curhatan terklasifikasi <i>Butuh Pertolongan Segera</i> ({int(n_darurat)} dari {total} entri).",
...
def generate_sna_insight(G):
    ...
    bullets = [
        f"Ekosistem akun penolong terdiri dari <b>{G.number_of_nodes()} akun</b> dan <b>{G.number_of_edges()} interaksi</b> (kepadatan jaringan: {density:.4f}).",
        f"<b>{n_sup_edges}</b> dari seluruh interaksi adalah respons suportif/semangat kepada pengirim curhatan.",
        f"Akun paling sentral (paling banyak terhubung): <b>@{top_acc}</b> — kemungkinan menjadi simpul utama support system.",
    ]
    return bullets
...
<div class="mw-hero">
  ...
  <p>Tema 7 FTI · Klasifikasi Urgensi · Klasterisasi Akar Masalah · Support System Network (SNA)</p>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if not SASTRAWI_AVAILABLE:
        st.warning("Paket **Sastrawi** tidak terpasang — stemming dilewati. Jalankan `pip install Sastrawi` untuk mengaktifkannya.")
    st.markdown("### ℹ️ Tentang")
    st.caption(
        "**Tema 7 — FTI**\n\n"
        "Monitoring Isu Kesehatan Mental Gen Z\n\n"
        "Komponen:\n"
        "- Klasifikasi Urgensi\n"
...
if uploaded_file:
    ...
    with st.spinner("🌸 Memproses data..."):
        ...
        clean_texts = df["text_preprocessed"].apply(
            lambda t: t if isinstance(t, str) and t.strip() else "teks"
        ).tolist()

        n_docs = len(clean_texts)
        n_topics_eff = max(1, min(n_topics, n_docs))
        if n_topics_eff < n_topics:
            st.info(f"ℹ️ Jumlah topik LDA disesuaikan ke **{n_topics_eff}** (data: {n_docs} baris).")
        dom_topics, topic_words, log_lik, perplexity, lda_model, bow_vec = run_lda(clean_texts, n_topics_eff)
        df["lda_topic"] = dom_topics
        topic_label_map = {i: f"Topik {i}: {', '.join(topic_words[i][:3])}" for i in range(n_topics_eff)}
        df["lda_topic_label"] = df["lda_topic"].map(topic_label_map)
        n_topics = n_topics_eff

        n_clusters_eff = max(2, min(n_clusters, n_docs - 1)) if n_docs > 2 else 1
        if n_clusters_eff < n_clusters:
            st.info(f"ℹ️ Jumlah klaster K-Means disesuaikan ke **{n_clusters_eff}** (data: {n_docs} baris).")
        km_labels, X_2d, km_final, k_range, wcss, sil, dbi, chi = run_kmeans(clean_texts, n_clusters_eff)
        df["kmeans_cluster"] = km_labels
        n_clusters = n_clusters_eff

        G = build_network(df, text_col, username_col, parent_col)

    # ---- KPI & Lampu Indikator Urgensi ----
    total       = len(df)
    n_darurat   = (df["urgensi"] == "🚨 Butuh Pertolongan Segera").sum()
    n_perhatian = (df["urgensi"] == "⚠️ Perlu Perhatian").sum()
    ...
    if pct_darurat >= 15:
        light_state = "red"
        status_text = "⚠️ SIAGA TINGGI"
    elif pct_darurat >= 5:
        light_state = "yellow"
        status_text = "👁️ PERLU DIPANTAU"
    else:
        light_state = "green"
        status_text = "✅ TERKENDALI"

    red_opacity    = 1.0 if light_state == "red"    else 0.15
    yellow_opacity = 1.0 if light_state == "yellow" else 0.15
    green_opacity  = 1.0 if light_state == "green"  else 0.15
    red_extra    = "box-shadow:0 0 30px 8px rgba(244,166,166,0.65);animation:pulse 1.6s infinite;" if light_state == "red"    else ""
    yellow_extra = "box-shadow:0 0 30px 8px rgba(255,217,160,0.6);"                                if light_state == "yellow" else ""
    green_extra  = "box-shadow:0 0 26px 6px rgba(184,227,216,0.6);"                                if light_state == "green"  else ""

    st.markdown("### 🚦 Lampu Indikator Urgensi Isu")
    col_light, col_kpi = st.columns([1, 2.5])

    with col_light:
        st.markdown(f"""
        <div class="mw-light-wrap">
          <div class="mw-light" style="background:{C_SOFTRED}; opacity:{red_opacity}; {red_extra}"></div>
          <div class="mw-light" style="background:{C_PEACH}; opacity:{yellow_opacity}; {yellow_extra}"></div>
          <div class="mw-light" style="background:{C_MINT}; opacity:{green_opacity}; {green_extra}"></div>
        </div>
    ...
    with col_kpi:
        ...
        k2.metric("🚨 Butuh Pertolongan", int(n_darurat), f"{pct_darurat:.1f}%")
    ...
    if n_darurat > 0:
        ...
        <b>⚠️ Peringatan Sistem:</b> Terdeteksi <b>{int(n_darurat)} curhatan berisiko tinggi</b> yang memerlukan perhatian segera.
        Harap lakukan tindak lanjut pada segmen ini.
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab0:
        ...
        st.caption("Setiap tweet melewati 7 tahap sebelum digunakan untuk pemodelan.")
        ...
        for label, val in ex_labels:
            st.markdown(f"**{label}**")
            st.markdown(f'<div class="mw-before-after">{val if val else "(kosong)"}</div>',
                        unsafe_allow_html=True)
            st.write("")
        ...
        with c2:
            ...
            fig_funnel.update_layout(**PLOTLY_LAYOUT, barmode="group",
                                     xaxis_title="", yaxis_title="Jumlah Kata")
        ...
        insight_card("Insight Preprocessing", [
            ...
            f"Setelah stopword removal, kata unik berkurang dari "
            f"<b>{stats_df.loc[stats_df['Tahap']=='Normalisasi','Kata Unik'].values[0]}</b> menjadi "
            f"<b>{stats_df.loc[stats_df['Tahap']=='Stopword Removal','Kata Unik'].values[0]}</b>.",
            "Stemming Sastrawi menyatukan variasi morfologis (mis. 'menangis' & 'tangisan') ke akar kata yang sama."
            if SASTRAWI_AVAILABLE else
    ...
    with tab1:
        ...
        c1.metric("Rata-rata Kata/Tweet", f"{df['jumlah_kata'].mean():.1f}")
        c2.metric("Median Kata/Tweet", f"{df['jumlah_kata'].median():.0f}")
        c3.metric("Rata-rata Karakter", f"{df['jumlah_karakter'].mean():.0f}")
        c4.metric("Tweet Terpanjang (kata)", f"{df['jumlah_kata'].max()}")

        st.markdown("#### Distribusi Panjang Tweet")
        c1, c2 = st.columns(2)
        with c1:
            fig_wc_len = px.histogram(df, x="jumlah_kata", nbins=30,
                                      color_discrete_sequence=[C_ACCENT])
            fig_wc_len.update_layout(**PLOTLY_LAYOUT, xaxis_title="Jumlah Kata",
                                     yaxis_title="Frekuensi")
            st.plotly_chart(fig_wc_len, use_container_width=True)
        with c2:
            fig_ch_len = px.histogram(df, x="jumlah_karakter", nbins=30,
                                      color_discrete_sequence=[C_MINT])
            fig_ch_len.update_layout(**PLOTLY_LAYOUT, xaxis_title="Jumlah Karakter",
                                     yaxis_title="Frekuensi")
        ...
        if ts_col and df["_ts_parsed"].notna().sum() > 0:
            ...
            with c1:
                ...
                fig_hour = px.bar(hour_count, x="Jam", y="Jumlah",
                                  color_discrete_sequence=[C_ACCENT_DUST])
                fig_hour.update_layout(**PLOTLY_LAYOUT, xaxis_title="Jam (0-23)",
            ...
            with c2:
                ...
                fig_date = px.line(date_count, x="Tanggal", y="Jumlah", markers=True,
                                   color_discrete_sequence=[C_ACCENT])
                fig_date.update_layout(**PLOTLY_LAYOUT, xaxis_title="Tanggal",
                                       yaxis_title="Jumlah Postingan")
        ...
        with c1:
            ...
            raw_top = pd.DataFrame(Counter(raw_words).most_common(15),
                                   columns=["Kata", "Frekuensi"])
            fig_raw_top = px.bar(raw_top.sort_values("Frekuensi"), x="Frekuensi", y="Kata",
                                  orientation="h", color="Frekuensi",
                                  color_continuous_scale=PASTEL_SCALE,
                                  title="Top Words — Sebelum Stopword Removal")
        ...
        with c2:
            ...
            if final_words:
                wc = WordCloud(width=900, height=420, background_color="white",
                               mode="RGB", colormap="RdPu", max_words=80
        ...
        if engagement_cols:
            ...
            top_eng = df.sort_values(top_col, ascending=False).head(5)[
                [text_col, top_col, "urgensi"]
            ]
            st.markdown(f"**Top 5 tweet dengan {top_col} tertinggi:**")
            st.dataframe(top_eng, use_container_width=True, hide_index=True)
        else:
            st.info("💡 Tidak ada kolom engagement terdeteksi di dataset ini.")

    # -------- TAB 2: KLASIFIKASI URGENSI --------
    with tab2:
        st.subheader("🚦 Klasifikasi Tingkat Urgensi Curhatan")
        st.caption(
            "Curhatan diklasifikasikan ke tiga level: "
            "**🚨 Butuh Pertolongan Segera** · **⚠️ Perlu Perhatian** · **💬 Curhat Ringan**"
        )
        ...
        if ts_col and df["_ts_parsed"].notna().sum() > 0:
            ...
            trend_pivot = trend_df.groupby(["periode", "urgensi"]).size().unstack(
            ...
            fig_trend.update_layout(**PLOTLY_LAYOUT, xaxis_title="Tanggal",
                                    yaxis_title="Jumlah Tweet")
            st.plotly_chart(fig_trend, use_container_width=True)

        insight_card("Insight Otomatis — Klasifikasi Urgensi", generate_urgency_insight(df))

    # -------- TAB 3: KLASTERISASI AKAR MASALAH --------
    with tab3:
        st.subheader("🧩 Klasterisasi Akar Masalah Penyebab Kecemasan Gen Z")
        st.caption(
            "Klaster utama: **Tekanan Akademik** · **Masalah Keluarga** · **Finansial** "
            "· Hubungan Sosial · Kesehatan Mental Umum"
        )
        ...
        c1, c2 = st.columns(2)
        with c1:
            # Grafik lingkaran klaster penyebab stres terbesar Gen Z (rancangan dosen)
            fig_kp = px.pie(klaster_count, names="Klaster", values="Jumlah", hole=0.42,
                            color_discrete_sequence=PASTEL_SEQ,
                            title="Grafik Lingkaran — Klaster Penyebab Stres Gen Z")
            fig_kp.update_traces(textposition="outside", textinfo="percent+label")
        ...
        with c2:
            ...
            fig_kb.update_layout(**PLOTLY_LAYOUT, coloraxis_showscale=False,
                                 xaxis_title="Jumlah Tweet", yaxis_title="")
            st.plotly_chart(fig_kb, use_container_width=True)

        st.subheader("K-Means Clustering — Pengelompokan Otomatis")
        ...
        with c1:
            ...
            fig_elbow.add_vline(x=n_clusters, line_dash="dash", line_color=C_ACCENT_DUST,
                                ...
                                annotation_font_color=C_ACCENT_DUST)
            fig_elbow.update_layout(**PLOTLY_LAYOUT, xaxis_title="Jumlah Klaster (K)",
        ...
        with c2:
            ...
            df_plot = pd.DataFrame({
                "x": X_2d[:, 0], "y": X_2d[:, 1],
                "Klaster": df["kmeans_cluster"].astype(str)
            })
            ...
            fig_scatter.add_trace(go.Scatter(
                ...
                marker=dict(symbol="x", size=16, color=C_TEXT,
        ...
        st.subheader("Kata Dominan per Klaster (Top-5)")
        ...
        for cluster_id in range(n_clusters):
            with cols_km[cluster_id]:
                ...
                n_docs_c = len(cluster_texts)
                words_str = " · ".join(top_words) if top_words else "—"
                st.markdown(f"""
                <div class="mw-card">
                <h4>Klaster {cluster_id}</h4>
                <p style="font-size:12px;">📄 {n_docs_c} dokumen</p>
        ...
        if text_blob.strip():
            wc2 = WordCloud(width=1100, height=380, background_color="white",
                            mode="RGB", colormap="RdPu", max_words=80).generate(text_blob)
        ...
        insight_card("Insight Otomatis — Klasterisasi", generate_kmeans_insight(df, sil, n_clusters))

    # -------- TAB 4: LDA TOPIC MODELING --------
    with tab4:
        st.subheader("🔬 LDA Topic Modeling — Trending Topic Curhatan Gen Z")

        col_a, col_b = st.columns(2)
        with col_a:
            st.markdown(f"""
            <div class="mw-card">
            <h4>Metrik Evaluasi LDA</h4>
            <p class="mw-mono-val">
        ...
        with col_b:
            ...
            Representasi: <b>Bag of Words (BoW)</b>.
            </p>
            </div>
            """, unsafe_allow_html=True)

        st.subheader("Kata Kunci Tiap Topik (Trending Topic)")
        ...
        topic_dist["Label"] = topic_dist["Topik"].map(
            lambda x: f"Topik {x}: {', '.join(topic_words[x][:2])}")
        ...
        if ts_col and df["_ts_parsed"].notna().sum() > 0:
            ...
            fig_lda_trend.update_layout(**PLOTLY_LAYOUT, xaxis_title="Tanggal",
                                        yaxis_title="Volume Isu")
    ...
    with tab5:
        ...
        st.caption(
            "Model dilatih memprediksi label **urgensi** dari teks, "
            "sebagai pembanding terhadap pendekatan rule-based leksikon."
        )
        ...
        if len(class_counts) < 2 or min(class_counts.values()) < 2:
            st.warning(
                "⚠️ Data terlalu sedikit / tidak seimbang antar kelas untuk melatih classifier yang andal. "
                "Tambahkan lebih banyak data agar evaluasi lebih bermakna."
            )
        else:
            model, ml_vectorizer, ml_classes, report, cm, acc, f1m, n_test = train_ml_classifier(
                clean_texts, labels_list
            )
            ...
            report_df = report_df.loc[
                [c for c in ml_classes if c in report_df.index]
            ...
            with c1:
                ...
                fig_f1 = px.bar(f1_per_class, x="Kelas", y="F1-Score", color="Kelas",
                                color_discrete_map=COLOR_MAP_URGENCY,
                                range_y=[0, 1], text="F1-Score")
            ...
            insight_card("Insight Otomatis — ML Classifier", [
                ...
                "Label berasal dari pendekatan leksikon — classifier ini berguna sebagai pembanding "
                "kecepatan dan konsistensi terhadap aturan leksikon, bukan sebagai ground truth absolut.",
            ])
    ...
    # -------- TAB 6: SNA --------
    with tab6:
        st.subheader("🕸️ Support System Network (SNA)")
        st.caption(
            "Visualisasi ekosistem akun penolong — siapa yang paling aktif "
            "memberikan respons positif/semangat kepada pengirim curhatan.\n\n"
            "Edge **pink** = interaksi biasa · Edge **mint/hijau** = respon suportif"
        )

        if G.number_of_nodes() == 0:
            st.warning(
                "Tidak ada relasi (@mention / reply) terdeteksi. "
                "Dataset ini mengandung `in_reply_to_screen_name` — "
                "pastikan kolom Reply-To sudah dipilih di konfigurasi kolom."
            )
        else:
            ...
            n_sup_edges = sum(1 for _, _, d in G.edges(data=True) if d.get("support"))
            g3.metric("Interaksi Supportif", n_sup_edges)

            st.subheader("📊 Centrality Metrics — Akun Kunci Support System")
            deg_cen = nx.degree_centrality(G)
            ...
            with ca:
                ...
                for acc_, val in top5_deg:
                    st.markdown(f"<span class='mw-mono-val'>@{acc_}: {val:.3f}</span>",
                                unsafe_allow_html=True)
                st.markdown("</div>", unsafe_allow_html=True)
            with cb:
                st.markdown("<div class='mw-card'><h4>Betweenness Centrality</h4>",
                            unsafe_allow_html=True)
                ...
                st.markdown("</div>", unsafe_allow_html=True)
            with cc:
                st.markdown("<div class='mw-card'><h4>Eigenvector Centrality</h4>",
                            unsafe_allow_html=True)
                if top5_eig:
            ...
            insight_card("Insight Otomatis — Support System Network", generate_sna_insight(G))
    ...
    with tab7:
        ...
        # tambahkan kolom engagement jika ada
        for ec in engagement_cols:
            if ec not in display_cols:
    ...
    st.markdown("""
    <div class="mw-footer">
    🌷 <b>MindWatch Dashboard</b> — Tema 7 FTI · Monitoring Isu Kesehatan Mental Gen Z
    </div>
    """, unsafe_allow_html=True)
    ...
    <p style="font-size:14.5px; max-width:640px; margin: 12px auto;">
    Upload file CSV hasil scraping Twitter/X atau platform media sosial lainnya.<br>
    Dataset <b>burnoutkuliah.csv</b> sudah didukung secara otomatis:<br>
    kolom <b>full_text</b>, <b>in_reply_to_screen_name</b>, <b>created_at</b>,
    <b>favorite_count</b>, <b>reply_count</b>, <b>retweet_count</b>, <b>quote_count</b>
    akan terdeteksi otomatis.
    </p>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("### ✨ Komponen Dashboard (Sesuai Instruksi Dosen)")
    f1, f2, f3, f4 = st.columns(4)
    features = [
        ("🚦", "Lampu Indikator Urgensi",
         "Sistem peringatan otomatis jika curhatan 'Butuh Pertolongan Segera' meningkat."),
        ("🧩", "Klasterisasi Akar Masalah",
         "Tekanan Akademik · Masalah Keluarga · Finansial · Hubungan Sosial · Kes. Mental."),
        ("🕸️", "Visualisasi Ekosistem Akun Penolong",
         "SNA: akun/komunitas paling aktif memberi respons positif kepada pengirim curhatan."),
        ("🤖", "LDA & ML Classifier",
         "Trending topic curhatan Gen Z dan klasifikasi urgensi berbasis machine learning."),
    ]
